...
public class UtilisateurController {
        ...
        alert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    ...
                    gestionUtilisateur.executerSQL("SET FOREIGN_KEY_CHECKS=0");
    
                    // Supprimer l'utilisateur de la base de données
                    gestionUtilisateur.supprimerUtilisateur(utilisateur.getId());
    
                    // Réactiver les contraintes de clé étrangère
                    gestionUtilisateur.executerSQL("SET FOREIGN_KEY_CHECKS=1");
    ...
    private void afficherFormulaireAjout() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/AjoutUtilisateur.fxml"));
            Parent root = loader.load();
            Stage stage = new Stage();
